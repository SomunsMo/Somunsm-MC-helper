...
const createWindow = () => {
    window = new BrowserWindow({
        minWidth: 800, minHeight: 520, icon: path.join(__dirname, "../public/favicon.ico"), webPreferences: {
            // 使用Node的path模块引入预加载层
            ...
            contextIsolation: true, // 渲染进程能否访问Node环境（为了安全，建议关闭 |但这不影响渲染层的使用）
            nodeIntegration: false, // 禁用同源策略,允许跨域
            webSecurity: false
        }, // 隐藏标题栏
        frame: false, // 禁止窗口拖动成自定义尺寸
        // resizable: false,
        // 隐藏窗口顶部菜单栏
        autoHideMenuBar: true, // 窗口底色
        // backgroundColor: "red"
    })
